...
public class GUIEmployeeDelete extends JDialog {
	...
	public static void main(String[] args) {
		try {
			GUIEmployeeDelete dialog = new GUIEmployeeDelete();
	 ...
	 * Create the dialog.
	 */
	public GUIEmployeeDelete() throws IOException{
Useful useful = new Useful();
		...
		btnUsu_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e){
				dispose();
				try {
					useful.deleteLine("pracownicy.txt", Integer.parseInt(deletePositionTextField.getText()) );
					
					JOptionPane.showMessageDialog(getContentPane(),"Pozycja zosta³a usuniêta");
					GUIEmployeeDelete dialog;
					try {
						dialog = new GUIEmployeeDelete();
						dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE);			
		...
		JLabel lblUsuwaniePracownika = new JLabel("Usuwanie pracownika");
		lblUsuwaniePracownika.setHorizontalAlignment(SwingConstants.CENTER);
		lblUsuwaniePracownika.setFont(new Font("Tahoma", Font.PLAIN, 30));
		lblUsuwaniePracownika.setBounds(70, 11, 364, 40);
		panel_2.add(lblUsuwaniePracownika);
	}
